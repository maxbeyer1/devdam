...
def analyze(baseurl):
  """
  Prompts the user for an asset id, and calls the web service
  to analyze the image, which returns a list of labels.
  ...
  try:
    ...
    for label in body["data"]:
      label_obj = jsons.load(label, Label)
    ...
    for label in sorted(labels, key=lambda x: x.name):
      print(f"{label.name} with {round(label.confidence)}% confidence")
...
def search(baseurl):
  """
  Prompts the user for a label to search for, and calls the web service
  to search for images with that label.
  ...
  try:
    ...
    # sort assets by asset id ascending
    sorted_assets = sorted(assets, key=lambda x: x["asset_id"])
        
    # print assets
    for asset in sorted_assets:
      asset_id = asset["asset_id"]
      confidence = asset["confidence"]
      print(f"asset {asset_id} with {round(confidence)}% confidence")
